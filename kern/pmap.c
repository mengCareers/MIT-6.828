...
mem_init(void)
{
	...
	// Permissions: kernel RW, user NONE
	// Your code goes here:
	boot_map_region(kern_pgdir, KERNBASE, (1 << 31) - KERNBASE + (1 << 31), 0, PTE_W);
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if (*pt & PTE_P) {
		...
		if (create == 1 && (newpt = page_alloc(ALLOC_ZERO)) != 0) {
			...
			physaddr_t pa = page2pa(newpt);
			pgdir[PDX(va)] = pa | PTE_U  | PTE_W | PTE_P;

			pt_addr_v = (pte_t *)KADDR(pa);
			return pt_addr_v + PTX(va);
		}
	}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	for (; va < end; va = ROUNDUP(va + PGSIZE, PGSIZE)) {
		// return a pointer to the page table entry (PTE) for linear address 'va'
